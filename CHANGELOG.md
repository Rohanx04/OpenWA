...
- **Send and chat-list failures now return a diagnostic `502` instead of a bare `500 Internal server error`.** When the WhatsApp engine threw a raw, non-HTTP fault while executing an operation —
  ...
  primitive-string throw — it escaped to NestJS's default handler as an opaque `500 Internal Server Error`. That is exactly the `OpenWA API HTTP 500: Internal server error` a downstream integration saw
